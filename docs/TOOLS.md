...
| WiFi Analyzer | `WifiManager.getScanResults()` re-read every 30 s (active `startScan()` throttled to the same cadence); filter tabs **Band → Channel → Security → Display** (rightmost); Display = **List** (one live row per BSSID — SSID, signal bar, dBm, channel, band, security; associated AP's row green; rows update in place, `(gone)` when the AP drops off) or **Channel** (overlap AP count per channel — an AP counts toward every channel whose center falls inside its occupied bandwidth, e.g. ch1 + ch3 APs both hit ch2; rows sorted by channel number — empty 2.4 GHz ch 1–14 always listed, `█` bar = count); target bar = free-text SSID **or** MAC substring filter (optional), chips AND; switching Display while running re-runs immediately; no target needed |

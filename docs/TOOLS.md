...
| WiFi Analyzer | `WifiManager.getScanResults()` re-read every 30 s (active `startScan()` throttled to the same cadence); one live row per BSSID — SSID, signal bar, dBm, channel, band, security; the associated AP's row is colored green; rows update in place, `(gone)` when the AP drops off; target bar = free-text SSID **or** MAC substring filter (optional), chips below filter band / channel / security (AND); no target needed |
